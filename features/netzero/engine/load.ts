...
type ScenarioClient = Pick<SupabaseClient, "from">;

async function fetchAll<T>(
  supabase: ScenarioClient,
  table: string,
...
export async function loadScenarioData(): Promise<ScenarioData> {
  try {
    return await loadScenarioDataFrom(await createClient());
  } catch (err) {
    return loadFailure(err);
  }
}

/** Same load through a caller-supplied client (the server's service-role client for /api/explain). */
export async function loadScenarioDataFrom(supabase: ScenarioClient): Promise<ScenarioData> {
  try {
...
function loadFailure(err: unknown): ScenarioData {
  console.error(err);
  return {
    companies: [],
    macRows: [],
